...
type PoPositionSpecial = Tables<"app_purchase_orders_positions_special"> & {
  "base_model": Pick<Tables<"app_products">, "id" |"bb_sku" | "supplier_sku" | "purchase_details">;
  "billbee_product": Pick<Tables<"app_products">, "id" |"bb_sku" | "inventory_cagtegory">;
  app_products?: Pick<Tables<"app_products">, "id" |"bb_sku" | "supplier_sku"> | null;
...
export function PoPositionsSpecialShowList({ items, title }: Props) {
  return (
                    ...
                    <Col span={18} style={{ minWidth: 0 }}>
                        <Space direction="horizontal" size={8} style={{ width: "100%" }}>
                            <Typography.Text strong style={{ display: "block" }} ellipsis>
                                <Link href={`../../artikel/anzeigen/${item.base_model?.id}`}>{item.base_model?.bb_sku ?? "bb_sku fehlt"}</Link>
                            </Typography.Text>
                            <PoItemStatusTag status={item.po_item_status as string || ""} />
                            {(item.app_orders?.bb_OrderNumber && item.app_orders?.app_customers?.bb_Name) && (<Link href={`../../kundenberatung/auftrag/${item.app_orders?.id}`}><Tag>{item.app_orders?.bb_OrderNumber ?? "Bestellnummer fehlt"} - {item.app_orders?.app_customers?.bb_Name ?? "Kundenname fehlt"}</Tag></Link>)}
                            <SketchConfirmButton
