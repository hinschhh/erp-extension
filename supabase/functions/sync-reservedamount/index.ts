// supabase/functions/sync-reservedamount/index.ts
...
function serverError(msg: string, err?: unknown) {
  console.error("[sync-reservedamount] ERROR:", msg, err ?? "");
...
function getNumber(n: unknown, fallback: number) {
  ...
  return Number.isFinite(x) && x > 0
    ? Math.min(1000, Math.max(1, Math.floor(x)))
    : fallback;
}
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (ct.includes("application/json")) {
        ...
        cursor =
          (body as any).cursor != null ? Number((body as any).cursor) : null;
      }
    } else {
    ...
    console.log("[sync-reservedamount] Input:", { limit, cursor });

    // Batch lesen
    const q = supabase
      .from("components")
      .select("id, billbee_product_id")
    ...
    if (fetchError) {
      console.error("[sync-reservedamount] DB-Select-Fehler:", fetchError);
      return serverError("Fehler beim Laden aus 'components'", fetchError);
    }
    ...
    for (const row of rows as Array<{
      id: number;
      billbee_product_id: string | null;
    }>) {
      processed++;

      const { id, billbee_product_id } = row;
      if (!billbee_product_id) {
        skipped++;
        console.warn(
          `[sync-reservedamount] id=${id} ohne billbee_product_id, skip`
        );
        continue;
      }

      const url = `${BILLBEE_BASE}/products/reservedamount?id=${encodeURIComponent(
        billbee_product_id
      )}&lookupBy=id`;
      ...
      while (true) {
        ...
        console.warn(
          `[sync-reservedamount] 429 für ${billbee_product_id}. Warte ${retryAfterSec}s (Versuch ${attempts})`
        );
      ...
      if (!res || !res.ok) {
        console.error(
          `[sync-reservedamount] Billbee-Error id=${id}, billbee=${billbee_product_id}:`,
      ...
      try {
        ...
        console.error(
          `[sync-reservedamount] JSON-Parse-Fehler billbee=${billbee_product_id}`,
          e
        );
        skipped++;
        continue;
      ...
      if (reservedAmount == null || !Number.isFinite(reservedAmount)) {
        console.warn(
          `[sync-reservedamount] Kein ReservedAmount billbee=${billbee_product_id}, skip`
        );
        skipped++;
        continue;
      }
      ...
      if (updateError) {
        console.error(`[sync-reservedamount] DB-Update-Fehler id=${id}`, updateError);
        skipped++;
        continue;
      }
    ...
    let nextCursor: string | null = hasMore ? String(lastId) : null;

    // Safety: falls der Cursor nicht vorankommt, Schleife beenden
    ...
    console.log("[sync-reservedamount] DONE:", payload);
